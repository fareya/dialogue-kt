...
def format_dialogue_mathdial(data, pred_label_name, include_previous_turn_types=False, single_turn = False):
    formatted_outputs = []
    labels = [] 
    ids = [] 
    ...
    for conversation in data: 
        ...
        for i, turn in enumerate(conversation):
            if single_turn and turn[pred_label_name]:
                ...
                if pred_label_name == "future_teacher_move_type":
                    ...
                    if turn['future_teacher_move_type']:
                        ...
                        labels.append(turn[pred_label_name])
                        ids.append(str(turn["id"]) + str(turn["turn"]))
...
def format_dialogue_anation(data, label_type = "teacher_move_type", add_prev_labels = False, single_turn = False): 
    formatted_outputs = []
    labels = []
    ids = [] 
    for i, conversation in enumerate(data):
        ...
        final_dialogue = "[BEGIN DIALOGUE]"
        for k in range(len(conversation)): 
            if single_turn and is_tutor_list[k]:
                formatted_outputs.append(dialogue[k])
                labels.append(tutor_labels[k])
...
class DialogueDatasetUnpacked(DatasetBase):
    def __init__(self, data: list, tokenizer, skip_first_turn: bool = False, model_name: str = None, output_key: str = None, include_previous_turn_types = False, dataset_name = MATHDIAL, single_turn = False):
        ...
        if dataset_name == MATHDIAL:
            formatted_outputs, labels, ids = format_dialogue_mathdial(data, output_key, include_previous_turn_types, single_turn)
        elif dataset_name == ANATION: 
            formatted_outputs, labels, ids = format_dialogue_anation(data, output_key, include_previous_turn_types, single_turn)
...
if __name__ == "__main__":
    ...
    PRED_LABEL_NAME = "teacher_move_type" # options: future_teacher_move_type, teacher_move_type, correctness_annotation, future_correctness_annotation, final_correctness
    print(f"Label Name: {PRED_LABEL_NAME}")
    MODEL_NAME = "meta-llama/Llama-3.2-3B-Instruct"
    DATASET_NAME = "anation"# anation, mathdial
    ...
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME) 
    test_dataset = DialogueDatasetUnpacked(test_data, tokenizer, output_key = PRED_LABEL_NAME, include_previous_turn_types=INCLUDE_PREV, dataset_name = DATASET_NAME, single_turn = SINGLE_TURN)
